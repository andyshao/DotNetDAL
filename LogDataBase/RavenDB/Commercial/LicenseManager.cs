    ...
    public class LicenseManager : IDisposable
    {
        ...
        private RSAParameters RSAParameters
        {
            get
            {
                ...
                string path = string.Concat(base_path, "\\", "RavenDB.public.json");

                using (var stream = File.OpenText(path))
                {
                    if (stream == null)
                        throw new InvalidOperationException("Could not find public key for the license");
                    publicKeyString = stream.ReadToEnd();
                }
        ...
        {
            Dictionary<string, object> licenseAttributes = new Dictionary<string, object>();
            ...
            {
                Expiration = new DateTime(3000, 12, 12),
        ...
        public async Task<LicenseSupportInfo> GetLicenseSupportInfo()
        {
            ...
            //using (var context = JsonOperationContext.ShortTermSingleUse())
            {
                ...
                return new LicenseSupportInfo()
                {
                    EndsAt = new DateTimeOffset(new DateTime(3000, 12, 12)),
                    Status = Status.ProductionSupport,
                };
            }
        }
