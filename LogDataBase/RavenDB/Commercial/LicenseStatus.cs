...
namespace Raven.Server.Commercial
{
    public class LicenseStatus
    {
        ...
        public DateTime? Expiration { get; set; } = new DateTime(3000, 12, 12); //=> GetValue<DateTime?>("expiration");
        ...
        public int MaxCores => Environment.ProcessorCount;//GetValue<int?>("cores") ?? 3;

        //public int MaxMemory =>int.Parse(Sparrow.LowMemory.MemoryInformation
        //    .GetMemoryInfo()
        //    .TotalPhysicalMemory
        //    .GetDoubleValue(Sparrow.SizeUnit.Gigabytes).ToString());//6; //GetValue<int?>("memory") ?? 8;
        public int MaxMemory
        {
            get
            {
                var str = Math.Round(Sparrow.LowMemory.MemoryInformation
                             .GetMemoryInfo()
                             .TotalPhysicalMemory
                             .GetDoubleValue(Sparrow.SizeUnit.Gigabytes)).ToString();
                int max = 0;
        ...
        private int MaxClusterSizeInternal => 0;

        public bool DistributedCluster => true;
