...
namespace Raven.Client.ServerWide
{
    ...
    public class InternalReplication : ReplicationNode
    {
        ...
        {
            if (other is InternalReplication publicNode)
            {
                return base.IsEqualTo(publicNode) &&
                       string.Equals(Url, publicNode.Url, StringComparison.OrdinalIgnoreCase);
            }
            return false;
        }
    ...
    public class DatabaseTopology
    {
        ...
        // Find changes in the connection of the public database group
        public static (HashSet<string> AddedDestinations, HashSet<string> RemovedDestiantions) FindChanges(List<ReplicationNode> oldDestinations, List<ReplicationNode> newDestinations)
        {
