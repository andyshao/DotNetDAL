        ...
        public Dictionary<TKey, TValue> GenericReadJson<TKey, TValue>(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            do
            {
                ...
                if (s != null)
                {
                    ...
                    {
                        DateTime time;
                        if (DateTime.TryParseExact(s, DateTimeFormatsToRead, CultureInfo.InvariantCulture,
                    ...
                    {
                        DateTimeOffset time;
                        if (DateTimeOffset.TryParseExact(s, DateTimeFormatsToRead, CultureInfo.InvariantCulture,
                                                         DateTimeStyles.RoundtripKind, out time))
                        {
                            key = time;
                        }
